...
class video{
	...
	function check(&$p){
		...
		$p['up']=move_upload($p['up']);
		
		if(strlen($p['up'])<1 && strlen($p['vurl'])<1)
			amsg(l('请填写视频地址或上传文件'),'w','$("#vurl").focus();');
	...
	function move($itemid,$to_channelid){
		...
		// add
		$sql=sql_insert($p);
		if($this->db->query("insert into ".PF.'video_'.$to_channelid.' '.$sql)){
