...
class picture{
	...
	function add($p){
		...
		is_array($pics[0]) or $pics[0]=array ();
		is_array($pics[1]) or $pics[1]=array ();
		
		$n_arr=array ();
		foreach($pics[0] as $key=>$val){
		...
		foreach($n_arr as $pic=>$title){
			if(strlen($pic)){
				if($_tmp=move_upload($pic))
					$_pics[$_tmp]=$title;
				else
					upload_del($pic);
			}
	...
	function move($itemid,$to_channelid){
		...
		// add
		$sql=sql_insert($p);
		if($this->db->query("insert into ".PF.'picture_'.$to_channelid.' '.$sql)){
