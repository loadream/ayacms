...
if($CHANNELS[$channelid]['module']!='article')
	return '{diy "article_thumb"} $channelid参数有误';
...
if(!in_array($number,array (
	...
	return '{diy "article_thumb"}$number参数有误';
...
$items=array ();
$rs=$db->query("SELECT * FROM ".PF."article_".$channelid." WHERE {$where} order by {$order} LIMIT 0,".$line*$number*$page);
...
<a <?php echo $blank?> href="<?php echo $item['url']?>" class="card">
                ...
                <?php if(!empty($date)){?>
                  <span class="pull-right">  <?php echo html($item['posttime'])?></span>
                  <?php }?>
