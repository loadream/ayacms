<?php
if($CHANNELS[$channelid]['module']!='picture')
	return '{diy "picture_textpic"}$channelid不存在';
...
if(!in_array($number,array (
	...
	return '{diy "picture_textpic"}$number参数有误';
...
$items=array ();
$rs=$db->query("SELECT * FROM ".PF."picture_".$channelid." WHERE {$where} order by {$order} LIMIT 0,".$number*$page);
...
<?php }?>
<a <?php echo $blank?> href="<?php echo $item['url']?>"><?php echo html($item['title'])?></a>
