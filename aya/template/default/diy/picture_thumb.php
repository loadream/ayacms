...
if($CHANNELS[$channelid]['module']!='picture')
	return '{diy "picture_thumb"} $channelid参数有误';
...
if(!in_array($number,array (
	...
	return '{diy "picture_thumb"}$number参数有误';
...
$items=array ();
$rs=$db->query("SELECT * FROM ".PF."picture_".$channelid." WHERE {$where} order by {$order} LIMIT 0,".$line*$number*$page);
...
<a <?php echo $blank?> href="<?php echo $item['url']?>" class="card">
                ...
                <span class="caption"><?php echo html($item['title'])?></span>
              </a>
